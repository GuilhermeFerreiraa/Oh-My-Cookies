    ...
    <div className="flex flex-col gap-y-2 w-[385px]">
      ...
      <div className="flex w-full relative">
        <input
          ...
          className={`block w-full p-4 rounded-lg border-2 outline-none sm:text-sm enabled:hover:border-gray-400 transition-all appearance-none ${
            hasError
              ? "border-brand-main focus:border-brand-main text-brand-main placeholder-brand-main"
              : "border-gray-200 focus:border-gray-300 text-gray-400 placeholder-gray-400"
          }`}
        />
        ...
        {type === "password" && (
          <button onClick={() => setShowPassword((prevState) => !prevState)}>
            <span className="absolute ease-in inset-y-0 right-4 flex items-center pl-2">
              {showPassword ? (
                <Image src={openEyes} className="h-5 w-5" alt="opened_eyes" />
              ) : (
                <Image src={closeEyes} className="h-5 w-5" alt="closed_eyes" />
              )}
            </span>
          </button>
        )}
